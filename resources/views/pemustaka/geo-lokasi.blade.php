...
@section('content')
    ...
    <script>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            retryBtn.style.margin = "10px auto 0";
            ...
            function successFn(position) {
                ...
                // Bangun polygon dari titik geolokasi (minimal 3 titik)
                const polygonCoords = geolocations.map(loc => [parseFloat(loc.latitude), parseFloat(loc
                    .longitude)]);
                ...
                if (polygonCoords.length >= 3) {
                    areaPolygon = L.polygon(polygonCoords, {
                        color: 'blue',
                        fillOpacity: 0.2
                    }).addTo(map);
                    ...
                    // Gunakan bounds untuk mengecek apakah user berada di dalam polygon
                    isInside = areaPolygon.getBounds().contains(userLatLng);
                }

                // Hitung jarak ke titik terdekat untuk tambahan validasi
                let minDistance = Infinity;
                for (const loc of geolocations) {
                    const distance = getDistanceInMeters(lat, lng, parseFloat(loc.latitude), parseFloat(loc
                        .longitude));
                    minDistance = Math.min(minDistance, distance);
                }

                // Bersihkan alert sebelumnya
                document.querySelectorAll('.geo-alert.bg-yellow-500, .geo-alert.bg-green-500').forEach(e => e
                    .remove());

                if (!isInside || minDistance > 10) {
                    Swal.fire({
                        icon: 'warning',
                        title: 'Lokasi di Luar Area',
                        html: `Jarak ke titik terdekat: <b>${Math.round(minDistance)} meter</b>.<br>Silakan periksa GPS Anda atau coba deteksi ulang.`,
                    ...
                    Swal.fire({
                        icon: 'success',
                        title: 'Lokasi Valid!',
                        html: `Jarak ke titik terdekat: <b>${Math.round(minDistance)} meter</b>.`,
